    ...
    public function xoahocvien($hocvien)
{
    $dbcon = DATABASE::connect();
    try {
        // Xóa dữ liệu từ bảng nguoidung trước
        $sql_nguoidung = "DELETE FROM nguoidung WHERE hocvien_id=:hocvien_id";
        $cmd_nguoidung = $dbcon->prepare($sql_nguoidung);
        $cmd_nguoidung->bindValue(":hocvien_id", $hocvien->id);
        $cmd_nguoidung->execute();
        
        // Sau đó xóa dữ liệu từ bảng hocvien
        $sql_hocvien = "DELETE FROM hocvien WHERE id=:id";
        $cmd_hocvien = $dbcon->prepare($sql_hocvien);
        $cmd_hocvien->bindValue(":id", $hocvien->id);
        $result = $cmd_hocvien->execute();
